...
def is_duplicated_number(three_digit):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    set_len = len(set(list(str(three_digit))))
    result = set_len != len(list(str(three_digit)))
    # ==================================
    return result
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    result = [0,0]
    user_input_number = list(str(user_input_number))
    random_number = list(str(random_number))

    for user, rand in zip(user_input_number, random_number):
        if user == rand :
            result[0] += 1
        elif user in random_number:
            result[1] += 1
...
def is_no(one_more_input):
    ...
    result = result = one_more_input.lower() == 'n' or one_more_input.lower() == 'no' 
    # ==================================
    return result
...
def main():
    ...
    while True:
        ...
        baseball = get_strikes_or_ball(user_input,random_number)
        print("Strikes : {} , Balls : {}".format(baseball[0],baseball[1]))
        if baseball[0] == 3:
            continue_game = input("You win, one more(Y/N)? ")
            while not is_yes(continue_game) and not is_no(continue_game):
                print('Wrong Input, Input again')
                continue_game = input("You win, one more(Y/N)? ")
            if is_yes(continue_game):
                random_number = str(get_not_duplicated_three_digit_number())
                print("Random Number is : ", random_number)
            elif is_no(continue_game):
